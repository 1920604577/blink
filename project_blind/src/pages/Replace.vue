<template>
    <div>
      <el-table :data="tableDatas" style="width: 100%">
          ...
          <el-table-column prop="invitationStatus" label="答复" width="150">
            <template slot-scope="scope">
                {{ scope.row.invitationStatus === 1 ? '接受' : '拒绝' }}
            </template>
        </el-table-column>
          <el-table-column label="操作" width="300">
              <template v-slot="scope">
                  <el-button type="primary" size="mini" @click="handleEdit(scope.row)">操作</el-button>
              </template>
          </el-table-column>
      </el-table>
  ...
  export default {
      ...
      methods: {
          ...
          async load() {
              this.QueryAllByUserIdBo.token = this.token;
              this.QueryAllByUserIdBo.userId = this.user.id;
              try {
                var object = await synRequestPost("/datingInvitations/queryAllByUserId", this.QueryAllByUserIdBo);
